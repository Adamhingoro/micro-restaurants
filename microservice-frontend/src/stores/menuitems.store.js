...
const GetAllMenuItemsByMenu = async (token , id) => {
    const MenuItemsInstance = axios.create({
        baseURL: `${api_hostname}/api/v0/menuitems/`,
        headers: {'Authorization': `Token ${token}`}
    });
...
const GetMenuItemById = async (token , id) => {
    const MenuItemsInstance = axios.create({
...
const GetMenuItemImagePUTURL = async (token , id) => {
    const MenuItemsInstance = axios.create({
        baseURL: `${api_hostname}/api/v0/menuitems/imageurl/${id}`,
        headers: {'Authorization': `Token ${token}`}
    });
    try {
        const response = await MenuItemsInstance.get('/');
...
export { GetAllMenuItems , GetMenuItemById , SaveMenuItem , DeleteMenuItemById , GetAllMenuItemsByMenu , GetMenuItemImagePUTURL}